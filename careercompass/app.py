...
def load_datasets(dataset_id):
    ...
    job_df['CombinedSkills'] = job_df[['TechnicalSkills', 'Frameworks', 'VersionControl', 'SoftSkills', 'Tools']]\
...
def switch_dataset():
    ...
    if dataset_id in DATA_FILES:
        ...
        session['current_dataset'] = dataset_id  # Save the dataset ID in the session
...
def artificial_intelligence():
    return render_template('2.html')  # Renders the AI content

@app.route('/Biomedical')
def biomedical():
    return render_template('3.html')  # Renders the Biomedical content

# Define other routes similarly for 4.html, 5.html, etc.
@app.route('/Chemical')
def chemical():
...
def index():
    selected_domain = request.args.get('selected_domain', 'None')  # Get the selected domain from query parameters
    if request.method == 'POST':
        skills = request.form.get('skills')
        # Include selected_domain if needed in the redirection
